...
{
    void* ptr = IOS_Alloc(IOS_HEAP_SHARED, 0x828);
    memset(ptr, 0, 0x828);
    return ptr;
}
...
int FSA_ReadDir(int fd, int handle, DirectoryEntry_t* out_data)
{
    uint8_t* iobuf = allocIobuf();
    uint32_t* inbuf = (uint32_t*)iobuf;
    uint32_t* outbuf = (uint32_t*)&iobuf[0x520];
    ...
    if(out_data) memcpy(out_data, &outbuf[1], sizeof(DirectoryEntry_t));
...
int FSA_OpenFile(int fd, const char* path, char* mode, int* outHandle)
{
    uint8_t* iobuf = allocIobuf();
    uint32_t* inbuf = (uint32_t*)iobuf;
    uint32_t* outbuf = (uint32_t*)&iobuf[0x520];
...
int FSA_StatFile(int fd, int handle, FileStat_t* out_data)
{
    uint8_t* iobuf = allocIobuf();
    uint32_t* inbuf = (uint32_t*)iobuf;
    uint32_t* outbuf = (uint32_t*)&iobuf[0x520];
    ...
    if(out_data) memcpy(out_data, &outbuf[1], sizeof(FileStat_t));
...
int FSA_GetStat(int fd, char *path, FileStat_t* out_data)
{
    uint8_t* iobuf = allocIobuf();
    uint32_t* inbuf = (uint32_t*)iobuf;
    uint32_t* outbuf = (uint32_t*)&iobuf[0x520];
